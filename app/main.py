...
def get_trading_account_by_id(account_id: int) -> Optional[TradingAccount]:
    """الحصول على حساب تداول بواسطة ID"""
    try:
        db = SessionLocal()
...
def update_trading_account(
    ...
    try:
        ...
        logger.exception("Failed to update trading account")
        return False
...
def delete_trading_account(account_id: int) -> bool:
    ...
    try:
        ...
        logger.exception("Failed to delete trading account")
        return False
# -------------------------------
...
async def show_edit_accounts_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, telegram_id: int, lang: str):
    """عرض قائمة حسابات التداول للتعديل"""
    user_data = get_subscriber_with_accounts(telegram_id)
    
    if not user_data or not user_data['trading_accounts']:
        if lang == "ar":
            text = "⚠️ لا توجد حسابات تداول للتعديل."
        else:
            text = "⚠️ No trading accounts to edit."
    ...
    if lang == "ar":
        ...
        choose_text = "اختر الحساب الذي تريد تعديله:"
    else:
        header_title = "✏️ Edit Trading Accounts"
        back_label = "🔙 Back to my data"
        choose_text = "Choose the account you want to edit:"
    ...
    keyboard = []
    for account in user_data['trading_accounts']:
        if lang == "ar":
            button_text = f"🏦 {account['broker_name']} - {account['account_number']}"
        else:
            button_text = f"🏦 {account['broker_name']} - {account['account_number']}"
    ...
    try:
        ...
        logger.exception("Failed to show edit accounts menu: %s", e)
        
async def refresh_user_accounts_message(telegram_id: int, lang: str):
    """تحديث رسالة بيانات المستخدم وحسابات التداول"""
    ref = get_form_ref(telegram_id)
    if not ref:
        return
    ...
    # زر تعديل الحسابات - يظهر فقط إذا كان هناك حسابات
    if user_data['trading_accounts']:
        edit_accounts_button = InlineKeyboardButton(edit_accounts_label, callback_data="edit_my_accounts")
        keyboard.append([edit_accounts_button])
    ...
    try:
        ...
        logger.exception("Failed to refresh user accounts message")
# ===============================
...
def webapp_edit_account(request: Request):
    ...
    if not account_id:
        return HTMLResponse(content="Account ID missing", status_code=400)
    ...
    labels = {
        ...
        "submit": "💾 حفظ التعديلات" if is_ar else "💾 Save Changes",
    ...
    if account.copy_start_date:
        try:
            if "T" in account.copy_start_date:
                copy_start_date_value = account.copy_start_date.split("T")[0]
            else:
      ...
      <div class="card">
        ...
        <div class="form-row">
          <div>
            ...
            <input id="account" placeholder="123456" value="{account.account_number or ''}" />
          </div>
          <div>
            <label>{labels['password']}</label>
            <input id="password" type="password" placeholder="••••••••" value="{account.password or ''}" />
          </div>
        </div>

        <label>{labels['server']}</label>
        <input id="server" placeholder="Oneroyal-Live" value="{account.server or ''}" />
      ...
      <script>
        ...
        async function submitForm(){{
          ...
          try{{
            ...
            if(resp.ok){{
              statusEl.style.color='green';
              statusEl.textContent=data.message||'تم التعديل بنجاح';
...
async def submit_edit_account(payload: dict = Body(...)):
    try:
        ...
        if not account_id:
            return JSONResponse(status_code=400, content={"error": "Account ID missing."})
        ...
        # تحديث الرسالة في التليجرام
        if telegram_id:
            await refresh_user_accounts_message(telegram_id, lang)

        message = "تم التعديل بنجاح" if lang == "ar" else "Updated successfully"
...
async def delete_account(payload: dict = Body(...)):
    try:
        ...
        message = "تم الحذف بنجاح" if lang == "ar" else "Deleted successfully"
        return JSONResponse(content={"message": message})
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # الخطوة 6: إضافة handler لتعديل الحسابات
    if q.data == "edit_my_accounts":
        await show_edit_accounts_menu(update, context, user_id, lang)
        return

    # الخطوة 8: إضافة handler للحساب المحدد
    if q.data.startswith("edit_account_"):
        account_id = q.data.replace("edit_account_", "")
        if WEBAPP_URL:
            url_with_params = f"{WEBAPP_URL}/edit-account?lang={lang}&account_id={account_id}"
            ...
            try:
                ...
                open_label = "✏️ افتح نموذج التعديل" if lang == "ar" else "✏️ Open Edit Form"
                keyboard = [[InlineKeyboardButton(open_label, web_app=WebAppInfo(url=url_with_params))]]
                reply_markup = InlineKeyboardMarkup(keyboard)
                
                await context.bot.send_message(
                    chat_id=user_id,
                    text="اضغط لفتح نموذج تعديل الحساب:" if lang == "ar" else "Click to open account edit form:",
                    reply_markup=reply_markup
                )
            except Exception:
                logger.exception("Failed to open account edit form")
        else:
            text = "⚠️ لا يمكن فتح النموذج حالياً." if lang == "ar" else "⚠️ Cannot open form at the moment."
            await q.edit_message_text(text)
        return
...
async def show_user_accounts(update: Update, context: ContextTypes.DEFAULT_TYPE, telegram_id: int, lang: str):
    ...
    # زر تعديل الحسابات - يظهر فقط إذا كان هناك حسابات
    if user_data['trading_accounts']:
        edit_accounts_label = "✏️ تعديل حساباتي" if lang == "ar" else "✏️ Edit my accounts"
