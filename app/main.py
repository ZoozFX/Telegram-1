...
Please review the submitted data or contact support.
        ...
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("OK", callback_data=f"delete_notification_{account_id}")]])
...
async def set_language(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Update lang in database if subscriber exists
    ...
    if subscriber:
        try:
            ...
            logger.exception(f"Failed to update lang in DB: {e}")
...
async def handle_delete_notification(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(data_parts) == 3 and data_parts[0] == "delete" and data_parts[1] == "notification":
        try:
...
# Add this handler
application.add_handler(CallbackQueryHandler(handle_delete_notification, pattern="^delete_notification_"))
